...
def match_keyword(data,path,page=None,company_name='',new_dict=False):
    if new_dict:
        data = get_new_data(data,c_name=company_name)
    ...
    for s1_counter,c_rel in enumerate(s1sec):
        ...
        if found!=True and c_rel['i_breakdown']:
            for breakdown in c_rel['i_breakdown'].split('##'):
                ...
                key_2 = extended_key %(breakdown.strip().lower()[:-1].replace('-','') if breakdown.strip()[-1] == 's' else breakdown.strip().lower().replace('-',''))
...
def save_data(obj,key_obj,path,page,sub_id,f_obj,item,sec,type,c_name,subsec=''):
    ...
    for i, j in enumerate(key_obj):
        y_key = get_year_name(j[0])#'q' + str(i + 1)
        if y_key:
            if sec=='s1sec':
                if not subsec:
                ...
                if gbc_obj and type=='synonym':
                        ...
                        if y_obj:
                            ...
                            key_dict = {'description': obj, 'year_date': j[0], 'y1': ('0' if j[1] in ['-','—'] else j[1]), 'pdf_page': page,
                                        'pdf_image_path': path}
                    ...
                    y_obj = year_data.objects.filter(id__in=gbc_obj.values_list(get_id, flat=True))
                    if y_obj:
                        if obj.lower() not in y_obj[0].description.lower().split(','):
                            ...
                            description = y_obj[0].description +','+obj
                    ...
                    key_dict = {'description':obj,'year_date': j[0], 'y1': ('0' if j[1] in ['-','—'] else j[1]),'pdf_page':page,'pdf_image_path':path}
                    query1 = year_data(**key_dict)
                    query1.save()
                    if not subsec:
                        gbc_dict = {'gbc_name': c_obj, 'section_id': f_obj,'subsection_id': sub_id, y_key: query1}
                    else:
                        gbc_dict = {'gbc_name': c_obj, 'section_id': f_obj, 's2section_id': sub_id,'subsection_id':subsec, y_key: query1}
...
def get_new_data(data,c_name):
    ...
    for year in year_list:
        ...
        year_exist = GbcData.objects.filter(gbc_name__company_name=c_name).filter(~Q(**{y_key:None}))
