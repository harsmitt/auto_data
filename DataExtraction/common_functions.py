...
def Create_blank_sheet(c_name,path,page):
    ...
    for sec in sec_obj:
        add_gbc(img_path, page, c_name,c_obj, sec,sub_obj=None,s2_obj=None)
        sub_obj = SubSection.objects.filter(section=sec)
        for sub in sub_obj:
            add_gbc(img_path, page, c_name, c_obj,sec,sub_obj=sub,s2_obj=None)
            s2_obj =S2Section.objects.filter(subsection=sub)
            if s2_obj:
                for s2 in s2_obj:
                    add_gbc(img_path,page,c_name,c_obj,sec,sub_obj=sub,s2_obj=s2)
...
def add_gbc(img_path,page,c_name,c_obj,sec,sub_obj=None,s2_obj=None):
    ...
    if gbc_obj:
        ...
        gbc_dict = {'gbc_name': c_obj, 'section': sec,
                    'q1': q_dict['q1'], 'subsection': sub_obj, 's2section': s2_obj,
                    'q2': q_dict['q2'], 'q3': q_dict['q3'], 'q4': q_dict['q4'],
                    'lrq':q_dict['lrq'],
                    'y1': y_dict['y1'],
                    'y2': y_dict['y2'], 'y3': y_dict['y3'], 'y4': y_dict['y4'],
                    }
        query1 = GbcData(**gbc_dict)
        query1.save()
        print ("add kr diya")
