...
def match_keyword_qtr(data,img_path,page=None,company_name='',new_dict=False):
    if new_dict:
    ...
    for i, j in enumerate(obj_val):
        ...
        if q_key:
            ...
            if gbc_obj and type=='synonym':
                ...
                q_obj = quarter_data.objects.filter(id__in=gbc_obj.values_list(get_id, flat=True))
                if q_obj:
                    if obj_name not in q_obj[0].description.split(','):
                        ...
                        description = q_obj[0].description +'##'+obj_name+'('+ str(j[1])+')' if q_obj[0].description else obj_name+'('+ str(j[1])+')'
