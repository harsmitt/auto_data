            ...
            for key in [key for key in db_obj['i_synonyms'].strip().split('##')  if key]:
                for b_comp in break_comp:
                    ...
                    if re_obj.match(b_comp.replace('-', '')) or re_obj2.match(b_comp.replace('-', '')):
                        if kwargs['extraction']=='pnl':
                            pdf_val = redefined_data(p_type= kwargs['p_type'],model = kwargs['model'],year_end=kwargs['year_end'], comp=kwargs['pdf_obj'],
                        ...
                        pdf_obj = kwargs['pdf_key_list'][kwargs['pdf_obj']] if kwargs['extraction'] !='pnl' else pdf_val
                        if 'insert' in db_obj:
...
def match_breakdown(**kwargs):
    try:
        ...
        for i_obj in [key for key in kwargs['obj_split']  if key]:
            # sim_key = similar_keyword_re % (extract_s(i_obj).strip().lower().replace('-',''))
            # key_2 = extended_key % (extract_s(i_obj).strip().lower().replace('-',''))
            # re_obj = re.compile(sim_key, re.I)
            # re_obj2 = re.compile(key_2, re.I)
            if match_breakdown_words(match_with=i_obj,pdf_obj=kwargs['p_obj'].replace('-', '')):#re_obj.match(kwargs['p_obj'].replace('-', '')) or re_obj2.match(kwargs['p_obj'].replace('-', '')):
                if kwargs['extraction'] == 'pnl':
                    pdf_val = redefined_data(p_type= kwargs['p_type'],model = kwargs['model'],year_end=kwargs['year_end'], comp=kwargs['p_obj'],
                                              pdf_obj=kwargs['pdf_key_list'][kwargs['p_obj']],pdf_type = kwargs['pdf_type'],
                                              d_obj=kwargs['d_obj'],type='breakdown', c_name=kwargs['c_name'])

                pdf_obj = kwargs['pdf_key_list'][kwargs['p_obj']] if kwargs['extraction'] != 'pnl' else pdf_val
        ...
        for loop, db_obj in enumerate(kwargs['db_key_list']):
            ...
            for synonym in [key for key in db_obj['i_synonyms'].strip().split('##') if key]:
                if match_all_synonym_words(match_with = synonym,pdf_obj=kwargs['pdf_obj']):
                    if kwargs['extraction']=='pnl':
                        pdf_val = redefined_data(p_type= kwargs['p_type'],model = kwargs['model'],comp=kwargs['pdf_obj'], year_end=kwargs['year_end'],
                                             pdf_obj=kwargs['pdf_key_list'][kwargs['pdf_obj']],
                                             d_obj=kwargs['db_key_list'][loop], type='breakdown',
                                             c_name=kwargs['c_name'],pdf_type = kwargs['pdf_type'])
