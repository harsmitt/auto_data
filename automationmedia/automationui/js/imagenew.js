...
album_app.item = function(data){
    ...
    this.moderation_status = m.prop(data.moderation_status || 0)
    this.deleted =m.prop(data.deleted || "")
    this.media_url = m.prop(data.media_url || '')
}

var AlbumApi ={
    get:function(){
        var ret = m.request({method:"GET",url:"/college-data-capture/api/album_api/", type: album_app.item}).then(function(data){return data})
...
album_app.controller = function(){
    ...
    {
        return App.Request.mdelete() &&  m.request({method:"POST", url:"/college-data-capture/delete/", data:{'tag':'collegealbum', 'id':this.id}
    }).then(common.reload)}
...
album_app.modal.controller = function(options) {
    ...
    ctrl.on_save = function(){
        ...
        if (validation == "validated"){
            ...
            if (ctrl.item().id()){
                data_send.append('album', ctrl.item().id())
            }
            data_send.append('album_name', ctrl.item()._name())
            data_send.append('deleted_pics', ctrl.deleted_pics())
            data_send.append('image', (ctrl.image_upload() || ""))
            url = "/college-data-capture/api/album_api/"
            common.showloading();
            m.request({method: "POST", url:url, data:data_send, serialize: function(val){return val}}).then(function(){ctrl.item(false); ctrl.deleted_pics([]); ctrl.image_upload(undefined); ctrl.error({}); window.location.reload();}).then(common.hideloading)
...
album_app.modal.view = function(ctrl) {
    ...
    if(album){
        var moderation_status = album.moderation_status()
    }
    var disabled = (moderation_status == 2) ? "disabled" : ""
    ...
    [m(".modal-box",{style:"display:block;"},[
        ...
        m(".modal-body",[
            m("div.form-group",name_error_class ? {class:"error"}:{},[
                ...
                m("input.form-control",{type:"text",oninput:on_input_name,value:album._name(), onfocus: function(){delete ctrl.error()["_name"]}, disabled: disabled}),
                name_error_class ? m(".message", common.error_message(name_error_class)): null
            ]),
            common.form_view_component.image_upload1(ctrl),
            m("h4","Pictures"),
                ...
                m("ul.gallery-list.flw100",
                    album.images().map(function(image){
                        ...
                        var close_onclick = function(){
                            // if((this.disabled) || (image.moderation_status == 2) ) {return}
                            var id = this.id
                        ...
                        return m("li",get_style(image.id),[
                            m("span",[
                                m("sup.sprite.close-white-icon", {onclick: close_onclick.bind({'id':image.id, 'disabled':disabled})}),
                            ]),
                            m("a",[
        ...
        return m(".col-md-4", 
                m(".create-album.album-box", [
                    m("a", {onclick:function(){ common.set_item(album_app)(album, ctrl.modal.item)}}, [
                        m("img[src='"+album.media_url()+"cdc/images/home-img.jpg']")
                    ])
                ]),
                m(".pull-left", [
                    m("h4", album._name()),
                    m("p.c9.fs-12.paddingBottom-0", album.count()+" Pictures"),
                        ...
                        album.deleted()?m("span.delete_text","Deleted"):'',
                        album.moderation_status()==2?m("span.delete_text","Approval Pending"):''
                ]),
